...
export class CoreComponent extends DestroyableComponent implements OnInit, OnDestroy, AfterViewInit {
	...
	private navigationItemsNL: NavigationItem[] = [
		...
		new NavigationItem('Configuratie', 'tool-02', undefined, false, [
			...
			new NavigationItem('Dokters', 'medical-circle', '/physician', false),
			new NavigationItem('Onderzoek', 'microscope', '/exam', false),
			new NavigationItem('Praktijkuren', 'clock', '/practice-hours', false),
			new NavigationItem('Prioritaire slots', 'calendar-date', '/priority-slots', false),
			new NavigationItem('E-mail sjablonen', 'mail-05', '/email-template', false),
	...
	public ngOnInit(): void {
			...
			next: (res) => {
				...
				res.forEach((element) => {
					...
					this.notifications.push(
						new NavigationItemEvent(element?.apmtId.toString(), date, element?.title, NavigationItemEventType.SUCCESS, element?.message, undefined, 'dd/MM/yyyy HH:mm'),
					);
				});
		...
		this.dashboardApiService.clearPosts$.pipe(takeUntil(this.destroy$$)).subscribe({
			next: (res) => {
				...
				res.forEach((element) => {
					this.messages.push(
						new NavigationItemEvent(element.id.toString(), DateTimeUtils.UTCDateToLocalDate(new Date(element.createdAt)), element?.message, undefined, undefined, undefined, 'dd/MM/yyyy HH:mm'),
					);
				});
